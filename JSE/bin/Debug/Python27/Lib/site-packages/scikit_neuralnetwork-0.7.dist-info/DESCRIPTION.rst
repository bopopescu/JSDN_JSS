...
* **Activation Functions â€”**
    * Nonlinear: ``Sigmoid``, ``Tanh``, ``Rectifier``.
    * Linear: ``Linear``, ``Gaussian``, ``Softmax``.
* **Layer Types â€”** ``Convolution`` (greyscale and color, 2D), ``Dense`` (standard, 1D).
* **Learning Rules â€”** ``sgd``, ``momentum``, ``nesterov``, ``adadelta``, ``adagrad``, ``rmsprop``.
* **Regularization â€”** ``L1``, ``L2`` and ``dropout``.
* **Dataset Formats â€”** ``numpy.ndarray``, ``scipy.sparse``, coming soon: iterators.
